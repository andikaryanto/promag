...
use Maatwebsite\Excel\Facades\Excel;
	...
	class AdminMaagMeterDatasController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Umur","name"=>"age"];
			$this->col[] = ["label"=>"Pola Makan","name"=>"diet"];
			$this->col[] = ["label"=>"Pola Hidup","name"=>"lifestyle"];
			//$this->col[] = ["label"=>"Gejala","name"=>"symptom"];
			$this->col[] = ["label"=>"Gejala","name"=>"symptom","callback_php"=>'\App\Http\Controllers\AdminMaagMeterDatasController::getGejalaData($row)'];
			
			//$this->col[] = ["label"=>"Waktu Gejala","name"=>"symptomtime"];
			$this->col[] = ["label"=>"Waktu Gejala","name"=>"symptomtime","callback_php"=>'\App\Http\Controllers\AdminMaagMeterDatasController::getWaktuGejalaData($row)'];
			
			$this->col[] = ["label"=>"Dampak Gejala","name"=>"symptomeffect"];
			$this->col[] = ["label"=>"Hasil","name"=>"result"];
			...
			$this->form[] = ['label'=>'Diet','name'=>'diet','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Lifestyle','name'=>'lifestyle','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Symptom','name'=>'symptom','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Symptomtime','name'=>'symptomtime','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Symptomeffect','name'=>'symptomeffect','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Result','name'=>'result','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			//$this->form[] = ["label"=>"Diet","name"=>"diet","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Lifestyle","name"=>"lifestyle","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Symptom","name"=>"symptom","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Symptomtime","name"=>"symptomtime","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Symptomeffect","name"=>"symptomeffect","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	    ...
	    */
			...
			switch ($filetype) {
				...
				case 'xls':
					...
					Excel::create($filename, function ($excel) use ($response) {
						$excel->setTitle($filename)->setCreator('report')->setCompany(CRUDBooster::getSetting('appname'));
						$excel->sheet('report', function ($sheet) use ($response) {
							$sheet->setOrientation($paperorientation);
							$sheet->loadview('vendor/crudbooster/report/maag_meter_data', $response);
						});
					})->export('xls');
					break;
				case 'csv':
		...
		static function  getGejalaData($row){

			
			$gejala_ids = explode("|",$row->symptom);
		...
		static function  getWaktuGejalaData($row){
			
			$gejala_ids = explode("|",$row->symptomtime);
